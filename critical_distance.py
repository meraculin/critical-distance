...
x, left, right, center_x_coordinate = util.openFiles(startLine, path)
...
for i in x:
    fitted_left.append(left[i-1])
    fitted_right.append(right[i-1])
...
data_5_adj = util.average_5_adj(fitted_left)
...
initial_state = util.convert_data(initial_state, left)
mean_initialstate = util.average(initial_state)

data_collision, collision_range = util.find_data_collision(data_5_adj, start_point, threshold, x)
...
if data_collision[0] == []:
    ...
    data_collision, collision_range = util.find_data_collision(data_collision, start_point, threshold, x)
...
if check == True:
    ...
    realdata_collision = util.convert_data(data_collision, left)    
    collision = util.convert_data(collision, left)
    
    mean_collision = util.average(realdata_collision)
    deltaX = mean_collision-mean_initialstate

    print(f"length of x, fitted_minima, collision, initial state= {len(x)}, {len(fitted_left)}, {len(collision)}, {len(initial_state)}")
    util.save_for_graph(x, fitted_left, collision, initial_state, ndata, path)
    ...
    collision = util.data_with_empty_list(data_collision, fitted_left)
    util.save_for_graph(x, fitted_left, collision, initial_state, ndata, path)