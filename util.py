...
def openFiles(startPoint,path):
    ...
    '''
    roi_T = f'data/roi_numbering/{path}_T.txt'
    roi_A = f'data/roi_numbering/{path}_A.txt'
    minima = f'data/localminima/{path}.txt'
    analyze_particle_T = f'data/analyze_particle/{path}_T.txt'
    analyze_particle_A = f'data/analyze_particle/{path}_A.txt'
    ...
    try:
        ...
        with open(analyze_particle_T, 'r') as file:
            ...
            for line in file:
                ...
                cx_coor_T.append((float(values[5])))
        ...
        with open(analyze_particle_A, 'r') as file:
            ...
            for line in file:
                ...
                values = line.strip().split('\t')
                # add the values to the appropriate list
                cx_coor_A.append((float(values[5])))
    

        return (xA, xT, left, right, cx_coor_T, cx_coor_A)
...
def fit_plot(path, rectangle):
    ...
    if rectangle[:5] == 'makeR':
        ...
        with open(f'data/analyze_particle/{path}.txt', 'r') as file:
            # Read the first line (the header) and split it on the tab character
            header = file.readline().strip().split('\t')
            # initialize empty lists to store the data
            x = []
